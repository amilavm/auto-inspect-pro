...
def get_final_assessment(files, claim_folder):
    ...
    #### restructure for combined parts ####
    if any("front_bumper" in s.values() for s in final_result_list):
        # res = [val for key, val in test_dict.items() if search_key in key]
        remove_list_index = []

        bumper_percentage_list = []
        service_types = []
        for i, s in enumerate(final_result_list, start=0):
            # print(i)
            for v in s.values():
                if 'front_bumper' in str(v):
                    bumper_percentage_list.append(s.get('damage_percentage'))
                    service_types.append(s.get('service_type'))
                    remove_list_index.append(i)

        for index in sorted(remove_list_index, reverse=True):
            del final_result_list[index]

        if 'replace' in service_types:
            new_service_type = 'replace'
        else:
            new_service_type = 'repair'

        combined_part_dict = {'part_name': 'front_bumper', 'damage_percentage': sum(bumper_percentage_list), 'service_type': new_service_type}
        final_result_list.append(combined_part_dict)
    ####
